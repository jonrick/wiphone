/***************************************************
  ...
  Modified by Andriy Makukha for ESP32 WiPhone project:
  - Added support for external RAM of ESP32 (20 Aug 2018)
  - Added support for 3-bit sprites (August 2018)
  - Added support for 3-bit RLE fonts (September 2018)
  - Added support for 3-bit RLE images (September 2018)
  - Added smooth font transparency (September 2018)
  - Added loading JPEG with tjpgd from ESP32's ROM (September 2018)
 ****************************************************/
...
#ifdef TFT_CS
  digitalWrite(TFT_CS, TFT_CS_DESEL); // Chip select high (inactive)
  //Serial.println("warnining: deselecting LCD turned off");
...
#define INTENSITY_16(c)         ( ((c & 0xF800) >> 11) + ((c & 0x7C0) >> 6) + (c & 0x1F) )
#define COLOR111_TO_565(c)      ( ((c & 4) ? 0xF800 : 0x00) | ((c & 2) ? 0x7E0 : 0) | ((c & 1) ? 0x1F : 0) )
#define COLOR111(c)                     (uint8_t)( ((c & 0xE000) ? 0x4 : 0) | ((c & 0x0700) ? 0x2 : 0) | ((c & 0x0018) ? 0x1 : 0) )
#define COLOR332(c)                     (uint8_t)( ((c & 0xE000)>>8) | ((c & 0x0700)>>6) | ((c & 0x0018)>>3) )
#define CEIL8_16(x)                     ((x+7) & 0xFFF8)
//#define CEIL24_16(x)          (x + (x % 24 ? 24 - (x % 24) : 0))
...
void TFT_eSPI::init(uint8_t tc)
{
  ...
  if (_booted)
  {
    ...
    SPI.begin(); // This will set HMISO to input
          ...
          //Serial.printf("TFT_SCLK, TFT_MISO, TFT_MOSI (OK) = %d, %d, %d\r\n", TFT_SCLK, TFT_MISO, TFT_MOSI);
      SPI.begin(TFT_SCLK, TFT_MISO, TFT_MOSI, -1);
    #else
          //Serial.printf("TFT_MISO (FAIL) = %d\r\n", TFT_MISO);
      SPI.begin();
    #endif
  #endif
#endif
...
***************************************************************************************/
uint16_t TFT_eSPI::readPixel(int16_t x0, int16_t y0)
{
#if defined(ESP32_PARALLEL)
...
***************************************************************************************/
void TFT_eSPI::readRect(uint32_t x, uint32_t y, uint16_t w, uint16_t h, uint16_t *data)
{
...
***************************************************************************************/
void TFT_eSPI::pushRect(uint32_t x, uint32_t y, uint16_t w, uint16_t h, uint16_t *data)
{
...
***************************************************************************************/
void TFT_eSPI::pushImage(int16_t x, int16_t y, uint16_t w, uint16_t h, uint16_t *data)
{
...
***************************************************************************************/
void TFT_eSPI::pushImage(int16_t x, int16_t y, uint16_t w, uint16_t h, uint16_t *data, uint16_t transp)
{
...
***************************************************************************************/
void TFT_eSPI::pushImage(int16_t x, int16_t y, uint16_t w, uint16_t h, const uint16_t *data)
{
  if ((x >= (int32_t)_width) || (y >= (int32_t)_height)) return;
...
***************************************************************************************/
void TFT_eSPI::pushImage(int16_t x, int16_t y, uint16_t w, uint16_t h, const uint16_t *data, uint16_t transp)
{
...
***************************************************************************************/
void TFT_eSPI::pushImage(int16_t x, int16_t y, uint16_t w, uint16_t h, uint8_t *data, uint8_t bpp)
{
  if ((x >= (int32_t)_width) || (y >= (int32_t)_height)) return;
  ...
  if ((x + w) > _width ) dw = _width  - x;                      // TODO: is this correct?
  if ((y + h) > _height) dh = _height - y;
  ...
  if (bpp==8)
  {
    ...
    while (dh--) {
      w =  CEIL8_16(w);
      int32_t len = dw;
      uint8_t* ptr = data;                      // TODO: optimize?
      uint8_t* linePtr = (uint8_t*)lineBuf;
      uint8_t  bits = 8;
      while (len>0) {
        if (len < 8) bits = len;
        ...
        *ptr++;         // proceed to next byte         TODO: optimize
        len -= 8;
      }
      pushColors(lineBuf, dw, false);
      dy++;
        ...
        uint16_t _bytewidth = (CEIL8_16(w)*3) >> 3; 
        for (uint16_t y=dy;y<dy+dh;y++) {
          uint16_t xb = 0;
          uint8_t* linePtr = (uint8_t*)lineBuf;
          for (uint16_t x=dx;x<dx+dw;x++) {
                // Unpack 8 pixels batch
                uint32_t b3 = 0;                // 24-bit data for 8 pixels
                uint16_t k = 3*(x>>3) + y*_bytewidth;
                b3 |= data[k++];
                b3 <<= 8;
                b3 |= data[k++];
                b3 <<= 8;
                b3 |= data[k];
                // Get pixel color
                uint32_t mask = (0xE00000 >> (3*(x & 0x7)));
                mask &= b3;
                mask >>= (3*(7-(x & 0x7)));
                // Convert to 16 bit
                uint16_t color16 = COLOR111_TO_565(mask);   // convert to 16-bit
                // Add to line buffer
                *linePtr++ = color16>>8;
                *linePtr++ = (uint8_t) color16;
                xb += 3;
          }
          pushColors(lineBuf, dw, false);
        }
...
***************************************************************************************/
void TFT_eSPI::pushImage(int16_t x, int16_t y, uint16_t w, uint16_t h, uint8_t *data, uint8_t transp, uint8_t bpp)
{
  if ((x >= (int32_t)_width) || (y >= (int32_t)_height)) return;
  ...
  else if (bpp==1)
  {
    ...
    while (dh--)
    {
      ...
      dy++;
        ...
        // Transparency is not implemented for 1-bit and 3-bit images
        while (dh--) {
      int32_t len = dw;
          uint16_t testColor = 0xFF0000;
      uint8_t* linePtr = (uint8_t*)lineBuf;
          while (len>0) {
                  *linePtr++ = testColor>>8;
                  *linePtr++ = testColor & 0xFF;
                  len--;
          }
          pushColors(lineBuf, dw, false);
          dy++;
...
int16_t SmoothFont::fitWordsLength(const char* string, uint16_t width, int8_t direction) 
{
  int16_t str_width = 0;                // current string width
  int16_t char_width = 0;               // current character width
  int16_t definitely_fit = 0;   // last fitting character (before whitespace or including fitting whitespaces)
  const char* p = string;
  const char* end = string + strlen(string);
  uint16_t n = 0;
  

  // TODO: only smooth fonts are supported
#ifdef SMOOTH_FONT
  if(this->isLoaded())
  {
        if (direction!=-1) {
                ...
                while (*p) {
                        n = 0;
                        uint16_t unicode = TFT_eSPI::decodeUTF8((uint8_t*) p, &n, end-p);
                        p += n;
                        if (unicode) {
                          if (unicode == 0x20) {
                                  char_width = this->spaceWidth;
                                  definitely_fit = p - string - 1;
                          } else if (unicode == '\n') {
                                  //log_v("str_width: %s, %d fit, %d width\r\n", string, p-string, str_width);
                                  return p-string;
                          } else {
                            if (unicode == '\t') definitely_fit = p - string - 1;           // another whitespace
                            uint16_t gNum = 0;
                            bool found = this->getUnicodeIndex(unicode, &gNum);
                            if (found)
                            {
                              char_width = this->gxAdvance[gNum];               // left padding (gdX) + gWidth + right padding
                              if(str_width == 0 && this->gdX[gNum] < 0) char_width += -this->gdX[gNum];         // add gdX if negative 
                            }
                            else char_width = this->spaceWidth + 1;
                          }
                          if (str_width + char_width > width) { 
                                  if (definitely_fit) {
                                        //log_v("str_width: %s, %d fit, %d width, next char = %d\r\n", string, definitely_fit, str_width, unicode);
                                        return definitely_fit;
                                  }
                                  //log_v("str_width: %s, %d fit, %d width, next = %d\r\n", string, p-string, str_width, unicode);
                                  return p-string-1;
                          }
                          str_width += char_width;
                          if (unicode == 0x20 || unicode == '\t') definitely_fit++;
                        }
                }
                //log_v("str_width: %s, %d fit, %d width\r\n", string, p-string, str_width);
                return p - string;
        } else {
                // TODO: only left-to-right is supported
        }
  }
#endif 
  return 0;
...
int16_t SmoothFont::fitTextLength(const char* string, uint16_t width, int8_t direction)
{
  int16_t fit = 0;
  int16_t str_width = 0;
  ...
  {
        if (direction!=-1) {
                // Not Right to left (Left to right or Centered)
                while (*p) {
                        uint16_t n = 0;
                        uint16_t unicode = TFT_eSPI::decodeUTF8((uint8_t*) p, &n, end-p);
                        p += n;
                        if (unicode) {
                          if (unicode == 0x20) char_width = this->spaceWidth;
                          else if (unicode == '\n') return fit+1;
                          else
                          {
                            uint16_t gNum = 0;
                            ...
                            {
                              char_width = this->gxAdvance[gNum];               // left padding (gdX) + gWidth + right padding
                              if(str_width == 0 && this->gdX[gNum] < 0) char_width -= this->gdX[gNum];
                            }
                            else char_width = this->spaceWidth + 1;
                          }
                          if (str_width + char_width > width) break;
                          str_width += char_width;
                          fit = p - string;
                        }
                }
                ...
                // Right to left
                // TODO: this logic will not work for actual UTF-8 text, fix it!
                // TODO: account for newlines
                // IDEA: can use 4-byte look-back
...
/*
Description: 
    draw only as many characters of the string as fit in the width (using
    current font and direction setting).   
    This works with different text centering (TL_DATUM, MC_DATUM, TR_DATUM,
    etc.). You need to change the position arguments accordingly.
*/
int16_t TFT_eSPI::drawFitString(const char *string, uint16_t width,  int16_t poX, int16_t poY) {
        // TODO: this is not optimal: 
        //   1) strlen calculated twice (here and inside fitTextLength)
        //   2) drawString recalculates textWidth
        int16_t fit = 0;
#ifdef SMOOTH_FONT
        if (smoothFont && smoothFont->isLoaded()) {
                if (!(textdatum & RIGHT_DATUM_FLAG)) {
                        fit = smoothFont->fitTextLength(string, width); 
                        if (fit>0) {
                                if (fit == strlen(string)) {
                                        drawString(string, poX, poY);
                                } else {
                                        char* buff = (char*) malloc(fit+1);
                                        memcpy(buff, string, fit);
                                        buff[fit] = '\0';
                                        drawString(buff, poX, poY);
                                        free(buff);
                                }
                        }
                } else {
                        // TODO: this will not work for UTF-8 strings, fix this!
                        fit = smoothFont->fitTextLength(string, width, -1);
                        if (fit>0) {
                                if (fit == strlen(string)) {
                                        drawString(string, poX, poY);
                                } else {
                                        char* buff = (char*) malloc(fit+1);
                                        memcpy(buff, string + (strlen(string)-fit), fit);
                                        buff[fit] = '\0';
                                        drawString(buff, poX, poY);
                                        free(buff);
                                }
...
#ifdef SMOOTH_FONT
  if(smoothFont && smoothFont->isLoaded())
          return smoothFont->textWidth(string);
#endif
...
{
#ifdef SMOOTH_FONT
  if(smoothFont && smoothFont->isLoaded()) return smoothFont->height();
#endif
...
***************************************************************************************/
void TFT_eSPI::drawChar(int16_t x, int16_t y, unsigned char c, uint32_t color, uint32_t bg, uint8_t size)
{
...
***************************************************************************************/
#if defined (ESP8266) && !defined (RPI_WRITE_STROBE)
void TFT_eSPI::drawPixel(int16_t x, int16_t y, uint32_t color)
{
...
***************************************************************************************/
void TFT_eSPI::pushColor(uint16_t color, int32_t len)
{
  spi_begin();
...
void TFT_eSPI::pushTransparent(int32_t len)
{
  pushColor(TFT_BLACK, len);
}

void TFT_eSPI::pushTransparent(uint16_t color, uint8_t alpha, int32_t len)
{
  pushColor(color, len);
}
...
void TFT_eSPI::drawLine(int16_t x0, int16_t y0, int16_t x1, int16_t y1, uint32_t color)
{
  spi_begin();
  inTransaction = true;
...
***************************************************************************************/
#if defined (ESP8266) && !defined (RPI_WRITE_STROBE)
void TFT_eSPI::drawFastVLine(int16_t x, int16_t y, int16_t h, uint32_t color)
{
...
***************************************************************************************/
#if defined (ESP8266) && !defined (RPI_WRITE_STROBE)
void TFT_eSPI::drawFastHLine(int16_t x, int16_t y, int16_t w, uint32_t color)
{
  // Rudimentary clipping
...
***************************************************************************************/
#if defined (ESP8266) && !defined (RPI_WRITE_STROBE)
void TFT_eSPI::fillRect(int16_t x, int16_t y, int16_t w, int16_t h, uint32_t color)
{
...
int16_t TFT_eSPI::drawString(const char *string, int poX, int poY, int font)
{
  ...
  if (textdatum || padX)
  {
    ...
    if (font!=1) {
      ...
      cheight = fontHeight(font);
        ...
        // Text alignment
        if (!(textdatum & TOP_DATUM_FLAG)) {
                if (textdatum & MIDDLE_DATUM_FLAG) 
                        poY -= (cheight>>1);
                else if (textdatum & BOTTOM_DATUM_FLAG) 
                        poY -= cheight;
                else if (textdatum & BASELINE_DATUM_FLAG)
                        poY -= baseline;
        }
        if (!(textdatum & LEFT_DATUM_FLAG)) {
                if (textdatum & CENTER_DATUM_FLAG) {
                        poX -= (cwidth>>1);
                        padding += 1;                           // TODO: what's the point?
                } else if (textdatum & RIGHT_DATUM_FLAG) {
                        poX -= cwidth;
                        padding += 2;
                }
        }
    ...
    // Check coordinates are OK, adjust if not
        // TODO: if TC_DATUM is printed at 0,0 it will automatically readjust position - CONFUSING
    if (poX < 0) { 
                poX = 0;
                //Serial.printf("WARNING: negative poX\r\n");
        }
    if (poX+cwidth > width()) {
                poX = width() - cwidth;
                //Serial.printf("WARNING: width exceeded %d\r\n", cwidth);
        }
    if (poY < 0) poY = 0;
  ...
  if(smoothFont && smoothFont->isLoaded())
  {
        #ifdef SMOOTH_FONT_FOUNDATION                   // define SMOOTH_FONT_FOUNDATION if you need this feature
    if (textcolor!=textbgcolor && smoothOpaque) {
...
void TFT_eSPI::setTextFont(SmoothFont* sFont)
{
        smoothFont = sFont;
        if (sFont) textfont = 0;
}
...
#endif
  //Serial.printf("pin_tft_miso = %d\r\n", tft_settings.pin_tft_miso);
}

bool TFT_eSPI::drawImage(const unsigned char* img, int len)
{
  return this->drawImage(img, len, 0, 0);
};

bool TFT_eSPI::drawImage(const unsigned char* img, int len, int16_t x, int16_t y)
{
  // Analyze "file header" to determine type of the image
  if (img[0]=='R' && img[1]=='L' && img[2]=='E' && img[3]=='3') 
    return this->drawImageRle3(img, len, x, y);
  if (img[0]=='I' && img[1]=='2' && img[2]=='5' && img[3]=='6') 
    return this->drawImageI256(img, len, x, y);
#ifdef ESP32
  if (img[0]==0xFF && img[1]==0xD8 && img[2]==0xFF)        // JPEG
    return display::load_jpg(img, len, this);              // TODO: account for x and y
#endif 
  return false;
}

bool TFT_eSPI::drawImage(IconRle3& icon, int16_t x, int16_t y)
{
  return this->drawImageRle3(icon.data, icon.size, x, y);
}

bool TFT_eSPI::drawImageRle3(const unsigned char* rle3, int len, int16_t x, int16_t y)
{
  // Header
  if (!(rle3[0]=='R' && rle3[1]=='L' && rle3[2]=='E' && rle3[3]=='3')) return false;
  const unsigned char* eos = rle3+len;
  rle3 += 4;

  // Dimensions
  uint32_t w = display::runDecodeNumber(&rle3);
  uint32_t h = display::runDecodeNumber(&rle3);
  uint8_t  c = (*rle3) & 0x7F;
  uint8_t *alpha = (*rle3) & 0x80 ? (uint8_t*) malloc(8) : NULL;                // alpha channel
  rle3 += 1;
  if (!w || !h || !c) return false;
  if (x+w>width() || y+h>height()) return false;                // TODO: allow going out of boundaries
  ...
  if (alpha) { 
    memset(alpha, 255, 8);
    for (i=0;i<c;i++) {
      color[i] = color565(rle3[0], rle3[1], rle3[2]);
      alpha[i] = rle3[3];
      rle3 += 4;
    }
  } else {
  ...
  while (rle3 < eos) {
    ...
    // Draw pixels
    if (alpha) {
          // Currently this supports only one fully transparent color.
          // (for TFT_eSPI class it will use black, TFT_eSprite - full transparency)
          // TODO: implement color blending for partial transparency (sprites only)
      if (alpha[c]==255)
        this->pushColor(color[c], cnt);
          else if (alpha[c]==0)
        this->pushTransparent(cnt);
      else
        this->pushTransparent(color[c], alpha[c], cnt);
    } else {
      this->pushColor(color[c], cnt);
    }
    total += cnt;
  ...
  if (alpha) free(alpha);  
  return (total == h*w) ? true : false;  
}

bool TFT_eSPI::drawImageI256(const unsigned char* dat, int len, int16_t x, int16_t y)
{
  // Header
  if (!(dat[0]=='I' && dat[1]=='2' && dat[2]=='5' && dat[3]=='6')) return false;
  const unsigned char* eos = dat+len;
  dat += 4;

  // Dimensions
  uint32_t w=display::runDecodeNumber(&dat);
  uint32_t h=display::runDecodeNumber(&dat);
  uint16_t c=*dat; dat++;
  if (!w || !h || !c) return false; 
  if (x+w>width() || y+h>height()) return false;                // TODO: allow going out of boundaries
  ...
  uint32_t i;
  for (i=0;i<=c;i++) {
    color[i] = this->color565(dat[0], dat[1], dat[2]);
    dat += 3;
  }
...
bool SmoothFont::isLoaded(void) {
  if (!fontLoadedType) return false;

  if(fontLoadedType==1 && !fontFile)
  {
    fontLoadedType = 0;
    return false;
  }
...
void SmoothFont::loadFont(String fontName)
{
  ...
  */
  ...
  gCount   = (uint16_t)readInt32(); // glyph count in file
                             readInt32(); // vlw encoder version - discard
  yAdvance = (uint16_t)readInt32(); // Font size in points, not pixels
                             readInt32(); // discard
  ascent   = (uint16_t)readInt32(); // top of "d"
  descent  = (uint16_t)readInt32(); // bottom of "p"
  ...
  while (gNum < gCount)
  {
    ...
    if (gdY[gNum] > maxAscent)
    {
      ...
      if (((gUnicode[gNum] > 0x20) && (gUnicode[gNum] < 0xA0) && (gUnicode[gNum] != 0x7F)) || (gUnicode[gNum] > 0xFF))
      {
        maxAscent   = gdY[gNum];
#ifdef SHOW_ASCENT_DESCENT
        log_d("Unicode = 0x%x maxAscent = %d", gUnicode[gNum], maxAscent);
#endif
      }
    }

    // Different glyph sets have different descent values not always based on "p", so get maximum glyph descent
    if (((int16_t)gHeight[gNum] - (int16_t)gdY[gNum]) > maxDescent)
    {
      // Avoid UTF coding values and characters that tend to give duff values
      if (((gUnicode[gNum] > 0x20) && (gUnicode[gNum] < 0xA0) && (gUnicode[gNum] != 0x7F)) || (gUnicode[gNum] > 0xFF))
      {
        maxDescent   = gHeight[gNum] - gdY[gNum];
#ifdef SHOW_ASCENT_DESCENT
        log_d("Unicode = 0x%x, maxDescent = %d", maxDescent);
#endif
      }
    }
  ...
  spaceWidth = (ascent + descent) * 2/7;  // Guess at space width
}

/***************************************************************************************
** Function name:           loadFont
** Description:             loads parameters from a 7SF file stored in array
*************************************************************************************x*/
void SmoothFont::loadFont(const unsigned char* data)
{
  // Similar to above, but loads font in custom 7-Shade Font format.
  ...
  gCount   = display::runDecodeNumber(&p); // glyph count in file
  yAdvance = display::runDecodeNumber(&p); // Font size in points, not pixels
  ascent   = display::runDecodeNumber(&p); // top of "d"
  descent  = display::runDecodeNumber(&p); // bottom of "p"
  ...
  for(uint16_t gNum = 0; gNum < gCount; gNum++) {
    ...
    gUnicode[gNum]  = (uint16_t)*((uint32_t*)p);        // Unicode code point value
        p += 4;
    gHeight[gNum]   = display::runDecodeNumber(&p);     // Height of glyph
    gWidth[gNum]    = display::runDecodeNumber(&p);     // Width of glyph
    gxAdvance[gNum] = display::runDecodeNumber(&p);     // xAdvance - to move x cursor
    gdY[gNum]       = (int8_t) display::runDecodeNumber(&p);    // y delta from baseline
    gdX[gNum]       = (int8_t) display::runDecodeNumber(&p);    // x delta from cursor

        // Ensure real maxDescent / maxAscent
    if (gdY[gNum] > maxAscent) {
      if (((gUnicode[gNum] > 0x20) && (gUnicode[gNum] < 0xA0) && (gUnicode[gNum] != 0x7F)) || (gUnicode[gNum] > 0xFF)) {
        maxAscent   = gdY[gNum];
        //Serial.print("Unicode = 0x"); Serial.print(gUnicode[gNum], HEX); Serial.print(", maxAscent = "); Serial.println(maxAscent);
      }
    }
    if (gHeight[gNum] - gdY[gNum] > maxDescent) {
      if (((gUnicode[gNum] > 0x20) && (gUnicode[gNum] < 0xA0) && (gUnicode[gNum] != 0x7F)) || (gUnicode[gNum] > 0xFF)) {
        maxDescent   = gHeight[gNum] - gdY[gNum];
        //Serial.print("Unicode = 0x"); Serial.print(gUnicode[gNum], HEX); Serial.print(", maxDescent = "); Serial.println(maxDescent);
      }
    }
  ...
  // Load palette
  int ic, ncolors = *((uint8_t*)p);
  p++;
  for (ic=0; ic<ncolors; ic++) {
          palette[ic] = *((uint8_t*)p);
          p++;
  }
  while (ic<sizeof(palette))
          palette[ic++] = 255;

  // Allocate bitmap data
  for(uint16_t gNum = 0; gNum < gCount; gNum++) {
    gBitmap[gNum] = (uint32_t) p;               // conversion from (uint8_t*) to (uint32_t)
        uint32_t n = display::runDecodeNumber(&p);
        p += n;
  }
...
/***************************************************************************************
** Function name:           unloadFont
** Description:             Delete the old glyph metrics and free up the memory
*************************************************************************************x*/
void SmoothFont::unloadFont( void )
{
  if (gUnicode)         { free(gUnicode); gUnicode = NULL; }
  if (gHeight)          { free(gHeight); gHeight = NULL; }
  if (gWidth)           { free(gWidth); gWidth = NULL; }
  if (gxAdvance)        { free(gxAdvance); gxAdvance = NULL; }
  if (gdY)                      { free(gdY); gdY = NULL; }
  if (gdX)                      { free(gdX); gdX = NULL; }
  if (gBitmap)          { free(gBitmap); gBitmap = NULL; }
...
uint16_t IconRle3::height() {
        if (_width==0 && _height==0)
                load();
        return _height;
}

uint16_t IconRle3::width() {
        if (_width==0 && _height==0)
                load();
        return _width;
}
...
void IconRle3::load() {
  const unsigned char* rle3 = data;
  if (size<4 || !(rle3[0]=='R' && rle3[1]=='L' && rle3[2]=='E' && rle3[3]=='3')) return;
  rle3 += 4;

  _width = display::runDecodeNumber(&rle3);
  _height = display::runDecodeNumber(&rle3);
}
...
*************************************************************************************x*/
uint16_t display::alphaBlend(uint8_t alpha, uint16_t fgc, uint16_t bgc)
{
...
*************************************************************************************x*/
uint32_t SmoothFont::readInt32(void)
{
  uint32_t val = 0;
...
uint32_t display::runDecodeNumber(const unsigned char** p) {
  uint32_t val = 0;
  uint8_t more = 0;
  do {
    val <<= 7;
...
void TFT_eSPI::drawGlyph(uint16_t code)                 // TODO: why is this repeated for eSprite?
{
  if (smoothFont == NULL) return;

  // Special cases: space and new line
  if (code < 0x21)
  {
    if (code == 0x20) {
      cursor_x += smoothFont->spaceWidth;
      return;
    }

    if (code == '\n') {
      cursor_x = 0;
      cursor_y += smoothFont->yAdvance;
  ...
  // Find the character
  uint16_t gNum = 0;
  bool found = smoothFont->getUnicodeIndex(code, &gNum);
  ...
  {
        // New line
    if (textwrapX && (cursor_x + smoothFont->gWidth[gNum] + smoothFont->gdX[gNum] > _width))
    {
      cursor_y += smoothFont->yAdvance;
      cursor_x = 0;
    }
    if (textwrapY && ((cursor_y + smoothFont->yAdvance) >= _height)) { cursor_y = 0; }
    if (cursor_x == 0) { cursor_x -= smoothFont->gdX[gNum]; }
        // Draw character
        if (smoothFont && smoothFont->fontLoadedType == 1) {
  
      smoothFont->fontFile.seek(smoothFont->gBitmap[gNum], fs::SeekSet); // This is taking >30ms for a significant position shift
  
      uint8_t pbuffer[smoothFont->gWidth[gNum]];
  
      uint16_t xs = 0;
      uint32_t dl = 0;
  
      int16_t cy = cursor_y + smoothFont->maxAscent - smoothFont->gdY[gNum];
      int16_t cx = cursor_x + smoothFont->gdX[gNum];
  
      for (int y = 0; y < smoothFont->gHeight[gNum]; y++)
      {
        smoothFont->fontFile.read(pbuffer, smoothFont->gWidth[gNum]); //<//
        for (int x = 0; x < smoothFont->gWidth[gNum]; x++)
        {
          ...
          if (pixel)
          {
            if (pixel != 0xFF)
            {
              ...
              drawPixel(x + cx, y + cy, display::alphaBlend(pixel, fg, bg));
            }
            else
            {
              if (dl==0) xs = x + cx;
        ...
        if (dl) { drawFastHLine( xs, y + cy, dl, fg); dl = 0; }
        ...
        } else if (smoothFont && smoothFont->fontLoadedType==2) {

          // Prepare palette: 7 shades of foreground (in order from darker to brighter)
          uint16_t gPalette[8];
          if (INTENSITY_16(fg) > INTENSITY_16(bg)) {
            for (int8_t ic=0; ic<8; ic++) {
                  gPalette[ic] = display::alphaBlend(smoothFont->palette[ic], fg, bg);
                  //Serial.printf("%d: 0x%02x -> 0x%04x\r\n", ic, smoothFont->palette[ic], gPalette[ic]);
                }
          } else {
            for (int8_t ic=7; ic>=0; ic--)
                  gPalette[7-ic] = display::alphaBlend(smoothFont->palette[ic], bg, fg);
          }
          ...
          if (smoothOpaque) {
            ...
            int16_t cy = cursor_y + smoothFont->maxAscent - smoothFont->gdY[gNum];
            int16_t cx = cursor_x + smoothFont->gdX[gNum];
            setWindow(cx, cy, cx + smoothFont->gWidth[gNum] - 1, cy + smoothFont->gHeight[gNum] - 1);
            
            // Decode pixels and push color
            const uint8_t* rle3 = (const uint8_t*) smoothFont->gBitmap[gNum];
            int32_t n = display::runDecodeNumber(&rle3);
            uint16_t cnt;
            while (n-- > 0) {
              // Decode 3-bit run-length encoded data
              uint8_t c = ((*rle3)>>5);      // color index     
              if ((*rle3) & 0x10) {
                // 2 bytes
                cnt = ((*rle3) & 0xF) << 8;
                rle3++;
              ...
              // Draw pixels
              pushColor(gPalette[c], cnt);
            }

      } else {

            // Smooth font transparency: try not to overwrite background (works slower)
                // WARNING: on some hardware FastHLine might cause problems here; use transparent fonts only with sprites
        ...
        int16_t cx = cursor_x + smoothFont->gdX[gNum];
            ...
            while (n-- > 0) {
              ...
              // Draw or skip pixels
                  // WARNING: this might draw only one pixel instead of a line, characters will look deformed
              while (cnt>0) { 
                if (x+cnt < smoothFont->gWidth[gNum]) {
                        if (smoothFont->palette[c]) 
                                        drawFastHLine( cx + x, cy + y, cnt, gPalette[c] );
                                        //drawFastHLine( cx + x, cy + y, cnt, c==7 ? 0xFFFF : 0xF100);
                                //else drawFastHLine( cx + x, cy + y, cnt, 0x0000 ); 
                                //if (code=='i') Serial.printf("line: %d\r\n", cnt);
                        x += cnt;
                        cnt = 0;
                } else {
                        if (smoothFont->palette[c]) 
                                        drawFastHLine( cx + x, cy + y, smoothFont->gWidth[gNum]-x, gPalette[c] );
                                        //drawFastHLine( cx + x, cy + y, smoothFont->gWidth[gNum]-x, c==7 ? 0x0000 : 0x0000);
                                //else drawFastHLine( cx + x, cy + y, smoothFont->gWidth[gNum]-x, 0x0000 );
                                //Serial.printf("skipped: %d\r\n", smoothFont->gWidth[gNum]-x);
                        cnt -= smoothFont->gWidth[gNum]-x;
                                //if (code=='i') Serial.printf("line & skip: %d\r\n", smoothFont->gWidth[gNum]-x);
                        x = 0;
                        y += 1;
                }
          }
    ...
    cursor_x += smoothFont->gxAdvance[gNum];
  }
  else
  {
    // Not a Unicode in font so draw a rectangle and move on cursor
    drawRect(cursor_x, cursor_y + smoothFont->maxAscent - smoothFont->ascent, smoothFont->spaceWidth, smoothFont->ascent, fg);
    cursor_x += smoothFont->spaceWidth + 1;
  }
...
{
  if(!smoothFont || !smoothFont->isLoaded()) return;
//  fontFile = SPIFFS.open( _gFontFilename, "r" );
  ...
  for (uint16_t i = 0; i < smoothFont->gCount; i++)
  {
    // Check if this will need a new screen
    if (cursorX + smoothFont->gdX[i] + smoothFont->gWidth[i] >= width())  {
      cursorX = -smoothFont->gdX[i];

      cursorY += smoothFont->yAdvance;
      if (cursorY + smoothFont->maxAscent + smoothFont->descent >= height()) {
        cursorX = -smoothFont->gdX[i];
        cursorY = 0;
    ...
    drawGlyph(smoothFont->gUnicode[i]);
    cursorX += smoothFont->gxAdvance[i];
...
void TFT_eSprite::cloneDataInto(TFT_eSprite* img) 
{
        // TODO: add checks for size
        img->cloneDataFrom((uint8_t*) frameBuffer(1));
}

void TFT_eSprite::cloneDataFrom(uint8_t* buffer)
{
        // This assumes that other sprite's buffer is identical in size to this sprite's buffer 
        int buffLen = 0;
        if (_bpp==16) {
                buffLen = (_dwidth * _dheight + 1)*sizeof(uint16_t);
                memcpy((uint8_t *) _img, buffer, buffLen);
        } else if (_bpp==8) {
                buffLen = (_dwidth * _dheight + 1)*sizeof(uint8_t);
                memcpy(_img8, buffer, buffLen);
        } else if (_bpp==1) {
                buffLen = _frames * (_bytewidth * _dheight + 1);
                memcpy(_img8, buffer, buffLen);
        } else if (_bpp==3) {
                buffLen = _bytewidth * _dheight + 1;            // no frames for 3-bit
                memcpy(_img8, buffer, buffLen);
        }
}
...
void* TFT_eSprite::createSprite(int16_t w, int16_t h, uint8_t frames)
{
  ...
  _iwidth  = _dwidth  = _awidth = _bytewidth = w;
  _iheight = _dheight = h;
  _frames  = frames;
  ...
  if (_bpp == 16)
  {
    _img8_1 = ( uint8_t*) esp32Calloc(w * h + 1, sizeof(uint16_t));
    _img8_2 = _img8_1;
  ...
  {
    _img8_1 = ( uint8_t*) esp32Calloc(w * h + 1, sizeof(uint8_t));
  ...
  else if (_bpp == 1)
  {
    ...
    w =  CEIL8_16(w);   // width should be the multiple of 8 bits to be compatible with epdpaint
    _iwidth = w;        // _iwidth is rounded up to be multiple of 8, so might not be = _dwidth
    _awidth = w;                // aligned width (maximum pixels can be stored in a row)
        _bytewidth = (w>>3);

    if (frames > 2) frames = 2; // Currently restricted to 2 frame buffers
    if (frames < 1) frames = 1;
    _img8 = ( uint8_t*) esp32Calloc(frames * _bytewidth * h + frames, sizeof(uint8_t)); // extra pixel added

    if (_img8)
    {
      _created = true;
      _img8_1 = _img8;
      _img8_2 = _img8 + ( _bytewidth * h + 1 );
      return _img8_1;
    }
  }
  ...
  else if (_bpp == 3)
  {
    //_dwidth   Display width+height in pixels always in rotation 0 orientation
    //_dheight  Not swapped for sprite rotations
    // Note: for 3bpp _iwidth and _iheight are swapped during Sprite rotations

    _iwidth = w;                                        // _iwidth is rounded up to be multiple of 8, so might not be = _dwidth
        _awidth = CEIL8_16(w);                  // aligned width (maximum pixels count that can be stored in a row)
        _bytewidth = (_awidth*3) >> 3;  // _bytewidth % 3 == 0

    _img8_1 = ( uint8_t*) esp32Calloc(_bytewidth * h + 1, sizeof(uint8_t)); // extra pixel added
...
void* TFT_eSprite::setColorDepth(int8_t b)
{
  ...
  {
    _created = false;
        //Serial.println("WARNING: recreating sprite");
...
{
  if (!_created ) {
      //Serial.println("WARNING: sprite not created to delete");
          return;
  }
...
{
  if (!_created) return;
//  TFT_eSprite* _spr = _tft->isSprite() ? (TFT_eSprite*) _tft : NULL;
  if (_bpp == 16) {
//        if (_spr)
//                _spr->pushImage(x, y, _iwidth, _iheight, _img);
//        else
//                _tft->pushImage(x, y, _iwidth, _iheight, _img);
        _tft->pushImage(x, y, _iwidth, _iheight, _img);
  }
  //else if (_spr)                      // TODO: pushing <16bit sprites to sprites not supported
  //      _spr->pushImage(x, y, _dwidth, _dheight, _img8, _bpp);
  else
          _tft->pushImage(x, y, _dwidth, _dheight, _img8, _bpp);
}

void TFT_eSprite::pushSpritePart(int16_t x, int16_t y, int16_t h)
{
  if (!_created) return;
  if (_bpp == 16) {
        _tft->pushImage(x, y, _iwidth, h, _img+(y*_iwidth));
  }
  else
        _tft->pushImage(x, y, _dwidth, h, _img8+(y*_dwidth), _bpp);
}
...
*************************************************************************************x*/
void TFT_eSprite::pushSprite(int16_t x, int16_t y, uint16_t transp)
{
  if (!_created) return;
  ...
  {
    transp = COLOR332(transp);
    _tft->pushImage(x, y, _dwidth, _dheight, _img8, (uint8_t)transp, _bpp);
  }
  else {
        _tft->pushImage(x, y, _dwidth, _dheight, _img8, 0, _bpp);               // no transparency for 1-bit and 3-bit images
  }
}

void TFT_eSprite::rotateXY(int16_t &x, int16_t &y) { 
  int16_t tx;
  if (_rotation == 1)
  {
    tx = x;
...
*************************************************************************************x*/
uint16_t TFT_eSprite::readPixel(int16_t x, int16_t y)
{
  ...
  // _bpp==1 and _bpp==3 coordinates
  rotateXY(x, y);
  if (_bpp==1)
  {
        return _img8[(x>>3) + y*_bytewidth] & (0x80 >> (x & 0x7));
  } 
  else if (_bpp==3) 
  {
        // Unpack 8 pixels batch
        ...
        mask >>= (3*(7-(x & 0x7)));
        // Convert to 16 bit
        return COLOR111_TO_565(mask);   // convert to 16-bit
  }
}
...
void  TFT_eSprite::pushImage(int16_t x, int16_t y, uint16_t w, uint16_t h, uint16_t *data)
{
  if ((x >= _iwidth) || (y >= _iheight) || (w == 0) || (h == 0) || !_created) return;

  if (_bpp == 16)
  {
        // Modify w & h to fit into this sprite
        if (x+w > _iwidth) {
                w = _iwidth-x;
        }
        if (y+h > _iheight) {
        ...
        // 
        uint32_t ypmx = (y + h) * _iwidth;
    for (uint32_t yp = y*_iwidth; yp < ypmx; yp += _iwidth)
    {
  ...
  else if (_bpp == 8)
  {
    for (uint32_t yp = y; yp < y + h; yp++)
    {
      for (uint32_t xp = x; xp < x + w; xp++)
      {
        uint16_t color = *data++;
        if(_iswapBytes) color = color<<8 | color>>8;
        _img8[xp + yp * _iwidth] = COLOR332(color);
      }
    }
  }
...
*************************************************************************************x*/
// TODO Need to add more area boundary checks
void  TFT_eSprite::pushImage(int16_t x, int16_t y, uint16_t w, uint16_t h, const uint16_t *data)
{
  if ((x >= _iwidth) || (y >= _iheight) || (w == 0) || (h == 0) || !_created) return;
...
*************************************************************************************x*/
void TFT_eSprite::setWindow(int16_t x0, int16_t y0, int16_t x1, int16_t y1)
{
  ...
  if (duff_coord)
  { // Point to that extra "off screen" pixel           // TODO: what is this inconsistency?
    _xs = 0;
    _ys = _iheight;
    _xe = 0;
    _ye = _iheight;
...
void TFT_eSprite::pushColor(uint16_t color)
{
  ...
  if (_bpp == 16)
    ...
    _img8[_xptr + _yptr * _iwidth] = COLOR332(color);
...
*************************************************************************************x*/
void TFT_eSprite::pushColor(uint16_t color, int32_t len)
{
  if (!_created ) return;
  ...
  if (_bpp == 16)
    ...
    pixelColor = COLOR332(color);

  // else: for 1-bit and 3-bit -> just pass color to writeColor without changes
...
void TFT_eSprite::pushTransparent(uint16_t color, uint8_t alpha, int32_t len)
{
  uint16_t readColor, lastColor, blendColor;
  readColor = lastColor = readPixel(_xptr, _yptr); 
  blendColor = display::alphaBlend(alpha, color, readColor);
  while (len-- > 0) { 
        readColor = readPixel(_xptr, _yptr); 
        if (lastColor != readColor) {
          blendColor = display::alphaBlend(alpha, color, readColor);
          lastColor = readColor;
        }
        drawPixel(_xptr, _yptr, blendColor);
...
void TFT_eSprite::pushTransparent(int32_t len)
{
  while (len-- > 0) {
    // Increment x
    _xptr++;
...
*************************************************************************************x*/
void TFT_eSprite::setScrollRect(int32_t x, int32_t y, uint16_t w, uint16_t h, uint16_t color)
{
...
void TFT_eSprite::scroll(int16_t dx, int16_t dy)
{
  ...
  uint16_t w  = _sw - abs(dx); // line width to copy
  uint16_t h  = _sh - abs(dy); // lines to copy
  ...
  else return; // TODO add scroll for 1 bpp and 3 bpp
...
void TFT_eSprite::fillSprite(uint32_t color)
{
  ...
  {
    memset(_img8, COLOR332(color), _iwidth * _iheight);
  }
  else if (_bpp == 1)
  {
  ...
  {
          int x;
          color = COLOR111(color);      // 3-bit color
          uint32_t b3 = color;          // 3-byte data for 8 pixels
          for(x=1;x<8;x++) {
                  color <<= 3;
                  b3 |= color;
          }
          int total = _dheight*_bytewidth;
          for (x=0;x<total;x+=3) {
                  _img8[x]   = (uint8_t) (b3>>16);
                  _img8[x+1] = (uint8_t) (b3>>8);
                  _img8[x+2] = (uint8_t) b3;
          }
  }
...
int16_t TFT_eSprite::width(void)
{
  ...
  if (_bpp >= 8) return _iwidth;

  // 1-bit and 3-bit
  if (_rotation == 1 || _rotation == 3) return _dheight;
  return _awidth;  // TODO: why do we use awidth here? not symmetric with height()
}
...
{
  if (!_created ) return 0;
  if (_bpp >= 8) return _iheight;

  // 1-bit and 3-bit
  if (_rotation == 1 || _rotation == 3) return _dwidth;         // _awidth?
  return _dheight;
}
...
/***************************************************************************************
** Function name:           setRotation
** Description:             Rotate coordinate frame for <8bpp sprites
*************************************************************************************x*/
...
/***************************************************************************************
** Function name:           getRotation
** Description:             Get rotation for <8bpp sprite
*************************************************************************************x*/
...
** Description:             push a single pixel at an arbitrary position
*************************************************************************************x*/
void TFT_eSprite::drawPixel(int16_t x, int16_t y, uint32_t color)
{
  ...
  {
    _img8[x+y*_iwidth] = COLOR332(color);
  }
  else if (_bpp == 1)
  {
        rotateXY(x, y);
    if (color) _img8[(x>>3) + y * _bytewidth] |=  (0x80 >> (x & 0x7));
    else       _img8[(x>>3) + y * _bytewidth] &= ~(0x80 >> (x & 0x7));
  }
  else if (_bpp == 3)
  {
        // THIS WILL WORK VERY SLOW, 3-bit storage is only optimized for space
        rotateXY(x, y);
        // Unpack
        uint32_t b3 = 0;                // 24-bit data for 8 pixels
        uint16_t k = 3*(x>>3) + y*_bytewidth;
        ...
        // Change pixel
        uint32_t mask = (0xE00000 >> (3*(x & 0x7)));
        b3 &= ~mask;            // clear 3 bits
        mask = (((uint32_t)COLOR111(color)) << 21) >> (3*(x & 0x7));
        b3 |= mask;                     // set 3 bits for actual color
        // Pack
        _img8[k--] = b3 & 0xFF;
        b3 >>= 8;
        _img8[k--] = b3 & 0xFF;
...
** Description:             draw a line between 2 arbitrary points
*************************************************************************************x*/
void TFT_eSprite::drawLine(int16_t x0, int16_t y0, int16_t x1, int16_t y1, uint32_t color)
{
  if (!_created ) return;
...
** Description:             draw a vertical line
*************************************************************************************x*/
void TFT_eSprite::drawFastVLine(int16_t x, int16_t y, int16_t h, uint32_t color)
{
  ...
  {
    while (h--) _img8[x + _iwidth * y++] = COLOR332(color);
  }
  else
  {
        // 1-bit and 3-bit sprites
    while (h--)
    {
      drawPixel(x, y, color);
      y++;
...
** Description:             draw a horizontal line
*************************************************************************************x*/
void TFT_eSprite::drawFastHLine(int16_t x, int16_t y, int16_t w, uint32_t color)
{
  ...
  {
    memset(_img8+_iwidth * y + x, COLOR332(color), w);
  }
  else
  {
        // 1-bit and 3-bit sprites
...
** Description:             draw a filled rectangle
*************************************************************************************x*/
void TFT_eSprite::fillRect(int16_t x, int16_t y, int16_t w, int16_t h, uint32_t color)
{
  if (!_created ) return;
...
#ifdef SMOOTH_FONT
  if(this->smoothFont && this->smoothFont->isLoaded())
  {
    uint16_t unicode = decodeUTF8(utf8);
    if (unicode < 32 && utf8 != '\n') return 0;
...
*************************************************************************************x*/
void TFT_eSprite::drawChar(int16_t x, int16_t y, unsigned char c, uint32_t color, uint32_t bg, uint8_t size)
{
  if (!_created ) return;
...
int16_t TFT_eSprite::drawChar(unsigned int uniCode, int x, int y, int font)
{
  ...
  // Font is not 2 and hence is RLE encoded
  {
    ...
    else if (_bpp == 8) color = COLOR332(textcolor);
    int px = 0, py = pY; // To hold character block start and end column and row values
    int pc = 0; // Pixel count
    uint8_t np = textsize * textsize; // Number of pixels in a drawn pixel
    uint8_t tnp = 0; // Temporary copy of np for while loop
...
void TFT_eSprite::drawGlyph(uint16_t code)
{
  ...
  {
    if (code == 0x20) {
      if (_created) this->cursor_x += this->smoothFont->spaceWidth;
      else this->cursor_x += this->smoothFont->spaceWidth;
      return;
    }
    ...
    if (code == '\n') {
      ...
      {
                this->cursor_x = 0;
                this->cursor_y += this->smoothFont->yAdvance;
  ...
  uint16_t gNum = 0;
  bool found = this->smoothFont->getUnicodeIndex(code, &gNum);

  SmoothFont*& font = this->smoothFont; 
  ...
  if (found)
  {
    ...
    if (newSprite)
    {
      //Serial.println("WARNING: implicit sprite creation at drawGlyph");
      createSprite(font->gWidth[gNum], font->yAdvance);
      if(bg) fillSprite(bg);
      this->cursor_x = -font->gdX[gNum];
      this->cursor_y = 0;
    }

        if (font->fontLoadedType==1) {

          // Font type = 8-bit VLW font (not tested)

      font->fontFile.seek(font->gBitmap[gNum], fs::SeekSet); // This is slow for a significant position shift!

      uint8_t pbuffer[font->gWidth[gNum]];

      uint16_t xs = 0;
      uint16_t dl = 0;

      for (int y = 0; y < font->gHeight[gNum]; y++)
      {
        font->fontFile.read(pbuffer, font->gWidth[gNum]);
        for (int x = 0; x < font->gWidth[gNum]; x++)
        {
          uint8_t pixel = pbuffer[x];
          if (pixel)
          {
            ...
            {
              if (dl) { 
                                drawFastHLine(xs, y + this->cursor_y + font->maxAscent - font->gdY[gNum], dl, fg);
                                dl = 0;
                        }
              if (pixel>127)
                                drawPixel(
                                                x + this->cursor_x + font->gdX[gNum], 
                                                y + this->cursor_y + font->maxAscent - font->gdY[gNum], 
                                                display::alphaBlend(pixel, fg, bg)
                                );
            }
            else
            {
              if (dl==0) xs = x + this->cursor_x + font->gdX[gNum];
              dl++;
            }
          }
          else
          {
            if (dl) { drawFastHLine( xs, y + this->cursor_y + font->maxAscent - font->gdY[gNum], dl, fg); dl = 0; }
          }
        }
        if (dl) { drawFastHLine( xs, y + this->cursor_y + font->maxAscent - font->gdY[gNum], dl, fg); dl = 0; }
      }

        } else if (font->fontLoadedType==2) {

          // Font type = 3-bit 7SF font
        
          // TODO: code to proceed to a new line?

          if (smoothOpaque) {

                // Prepare palette: 7 shades of foreground
                uint16_t gPalette[8];
                if (INTENSITY_16(fg) > INTENSITY_16(bg)) {
                  for (int8_t ic=0; ic<8; ic++)
                    gPalette[ic] = display::alphaBlend(font->palette[ic], fg, bg);
                } else {
                  for (int8_t ic=7; ic>=0; ic--)
                    gPalette[7-ic] = display::alphaBlend(font->palette[ic], bg, fg);
                }

                // Set window
        int16_t cy = cursor_y + font->maxAscent - font->gdY[gNum];
        int16_t cx = cursor_x + font->gdX[gNum];
                setWindow(cx, cy, cx + font->gWidth[gNum] - 1, cy + font->gHeight[gNum] - 1);
  
                // Decode pixels and push color
                const uint8_t* rle3 = (const uint8_t*) font->gBitmap[gNum];
                int32_t n = display::runDecodeNumber(&rle3);
                uint16_t cnt;
                while (n-- > 0) {
                  // Decode 3-bit run-length encoded data
        ...
        int16_t cx = cursor_x + smoothFont->gdX[gNum];
                ...
                bool bright = INTENSITY_16(fg) > 46;
            const uint8_t* rle3 = (const uint8_t*) smoothFont->gBitmap[gNum];
            int32_t n = display::runDecodeNumber(&rle3);
            int16_t cnt;
            int y = 0, x = 0;
            while (n-- > 0) {
              ...
              // Draw or skip pixels
                  if (smoothFont->palette[c]) {
                        // Blend with current pixels
                        while (cnt-->0) { 
                          uint16_t cur = this->readPixel( cx + x, cy + y );
                          uint16_t col = bright ? display::alphaBlend(smoothFont->palette[c], fg, cur) :
                                                                          display::alphaBlend(smoothFont->palette[7-c], cur, fg);
                          this->drawPixel( cx + x, cy + y, col );
                          if (++x >= smoothFont->gWidth[gNum]) {
                                x = 0;
                                y++;
                          }
                        }
    ...
    if (newSprite)
    {
      pushSprite(this->cursor_x + font->gdX[gNum], this->cursor_y, bg);
      deleteSprite();
      this->cursor_x += font->gxAdvance[gNum];
    }
    else this->cursor_x += font->gxAdvance[gNum];
  }
  else
  {
    // Not a Unicode in font so draw a rectangle and move on cursor
    drawRect(this->cursor_x, this->cursor_y + font->maxAscent - font->ascent, font->spaceWidth, font->ascent, fg);
    this->cursor_x += font->spaceWidth + 1;
  }
}
...
{
  if(!this->smoothFont || !this->smoothFont->isLoaded()) return;
...
** Description:             Write a string to the sprite cursor position
*************************************************************************************x*/
void TFT_eSprite::printToSprite(const char *cbuffer, int len) //String string)
{
  if(!this->smoothFont || !this->smoothFont->isLoaded()) return;
  ...
  if (newSprite)
  {
    ...
    while (n < len)
    {
      ...
      if (this->smoothFont->getUnicodeIndex(unicode, &index))
      {
        if (n == 0) sWidth -= this->smoothFont->gdX[index];
        if (n == len-1) sWidth += ( this->smoothFont->gWidth[index] + this->smoothFont->gdX[index]);
        else sWidth += this->smoothFont->gxAdvance[index];
      }
      else sWidth += this->smoothFont->spaceWidth + 1;
    }

    //Serial.println("WARNING: implicit sprite creation at printToSprite");
    createSprite(sWidth, this->smoothFont->yAdvance);
...
{
  bool newSprite = !_created;
  int16_t sWidth = this->smoothFont->gWidth[index];
  ...
  if (newSprite)
  {
    ...
    drawGlyph(this->smoothFont->gUnicode[index]);

    pushSprite(x + this->smoothFont->gdX[index], y, this->textbgcolor);
    deleteSprite();
  }

  else drawGlyph(this->smoothFont->gUnicode[index]);

  return this->smoothFont->gxAdvance[index];
}
#endif // SMOOTH_FONT


void TFT_eSprite::mirror() {
  if (!_created) return;
  if (_bpp == 16) {
    auto p = _img;
    for (auto j = 0; j < _iheight; j++) {
      for (auto i = 0; i < _iwidth/2; i++) {
        uint16_t tmp = *(p + _iwidth - 1 - i);
        *(p + _iwidth - 1 - i) = *(p + i);
        *(p + i) = tmp;
      }
      p += _iwidth;
...
void* TFT_eSprite::esp32Calloc(size_t nmemb, size_t size) {
        void* p = calloc(nmemb, size);
#ifdef ESP32
        if (p==NULL)
                // Try to allocate memory in external RAM
                p = heap_caps_malloc(size*nmemb, ((size==1) ? MALLOC_CAP_8BIT : MALLOC_CAP_32BIT) | MALLOC_CAP_SPIRAM);
#endif // ESP32
        return p;
}

#ifdef ESP32 
TFT_eSPI* display::jpgReceiver = NULL;
const unsigned char* display::jpgImg = NULL;
...
)
{
  ...
  if (buff) {
    ...
    rb = (display::jpgPos + nd <= display::jpgSize) ? nd : display::jpgSize - display::jpgPos;
    memcpy(buff, display::jpgImg + display::jpgPos, rb);
    display::jpgPos += rb;
    return rb;  /* Returns number of bytes could be read */
  }
  /* Skip nd bytes on the input stream */
  if (display::jpgPos + nd <= display::jpgSize) {
    display::jpgPos += nd;
    return nd;
  } 
  return 0;
...
static UINT display::tjd_output (
...
//  if (!rect->left) {
//    return 0;   /* Abort to decompression */
//  }
  ...
  // Text
//  DEBUG0("L: "); DEBUG0(rect->left);
//  DEBUG0(", R: "); DEBUG0(rect->right);
...
//  DEBUG0(", C: "); DEBUG0(color, HEX);
  ...
  //tft.fillRect(rect->left, rect->top, w, h, color);
  int x,y,w,h,j=0;
  w = rect->right-rect->left+1;
  h = rect->bottom-rect->top+1;
  for(y=0;y<h;y++) {
    for(x=0;x<w;x++) {
      uint16_t color = display::jpgReceiver->color565(b[j++], b[j++], b[j++]);
      display::jpgReceiver->pushColor(color);
    }
  }
...
)
{
    ...
    rc = jd_prepare(&jd, display::tjd_input, (void*) display::jpgDecodeBuff, JPG_BUFF_SIZE, 0);
    if (rc == JDR_OK) {

        /* Determine scale down factor */
        for (scale = 0; scale < 3; scale++) {
            if ((jd.width >> scale) <= screen->width() && (jd.height >> scale) <= screen->height()) break;
        }

        /* Display size information at bottom of screen */
        log_v("WxH: %dx%d, scale: %d", jd.width, jd.height, 1<<scale);

        /* Start to decompress the JPEG file */
        rc = jd_decomp(&jd, display::tjd_output, scale); /* Start to decompress */
